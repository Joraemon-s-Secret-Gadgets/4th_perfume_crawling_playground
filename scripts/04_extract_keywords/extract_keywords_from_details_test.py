...
# 3. 메인 프로세서
# ==========================================
async def process_file(input_file: str, limit: int = 5):
    ...
    # 테스트를 위해 상위 n개만 처리
    test_targets = perfumes[:limit]

    print(f"🚀 {input_file} 분석 시작 (대상: {len(test_targets)}개)")

    enriched_results = []
    for perfume in test_targets:
        # 동기 호출을 순차적으로 수행 (API 레이트 리미트 고려)
        enriched_item = enricher.enrich(perfume)
    ...
    print(f"💾 가공 완료 및 저장: {output_file}")
...
if __name__ == "__main__":
    # 테스트 실행 (Bvlgari 정제 파일 기준 상위 3개)
    asyncio.run(process_file("Bvlgari_final_data_refined.json", limit=3))
