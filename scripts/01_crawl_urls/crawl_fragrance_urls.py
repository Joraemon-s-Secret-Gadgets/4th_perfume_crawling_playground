...
async def crawl_brands_links():
    # 처리할 브랜드 설정 (이름과 총 페이지 수)
    brands_to_crawl = [{"name": "bvlgari", "pages": 4}, {"name": "Dior", "pages": 7}]
    ...
    async with async_playwright() as p:
        ...
        for brand_info in brands_to_crawl:
            brand_name = brand_info["name"]
            total_pages = brand_info["pages"]
            ...
            for current_page in range(1, total_pages + 1):
                print(
                    f"--- {brand_name} : {current_page} / {total_pages} 페이지 수집 중 ---"
                )
                ...
                print(
                    f"> {current_page}페이지에서 {current_page_count}개 추출 완료 (누적: {len(all_links)}개)"
                )
            ...
            file_name = f"{brand_name}_links.json"
        ...
        await browser.close()
        print("\n✨ 모든 브랜드 링크 수집 작업이 완료되었습니다.")


if __name__ == "__main__":
    asyncio.run(crawl_brands_links())
